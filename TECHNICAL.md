...
| `autoCQPlateauMinSavePercent` | `5` | How much smaller the file has to actually get before that budget may be spent, in percent of the current pick's own sample. A source that is already squeezed dry hardly shrinks across several CQ steps — measured on one of them, climbing four steps cost 0.7 VMAF and bought 1 % of file size, while a comparable source with room left gave 8 % over the same steps. Both hold the VMAF floor; only the size tells them apart, which is why a smaller `autoCQPlateauTolerance` cannot do this job — it would block the good case along with the pointless one. The sample sizes are already on disk, so the check costs no extra analysis time; `0` spends the budget no matter how little it buys |
| `autoCQMaxSourcePercent` | `45` | Spending limit for the search, as a share of the source bitrate: never use more than 45 % of what the source itself spends; the search then takes the cheapest CQ that still fits and says so in the log. Valid: `0` (off), or `10`–`100` |
...
- **Trust, but verify.** The interpolated pick is always confirmed with one extra real measurement; on a miss it steps down along the measured slope. A **saturation brake** detects pre-compressed sources whose quality plateaus below the target, and a **plateau climb** then probes higher CQ levels with real measurements — on such sources the low CQ steps often just ride the bitrate cap, so the climb routinely cuts a third of the file size at no visible cost. Since 1.32.0 the climb also asks what a rung actually *buys*: it gives up quality only when the file measurably shrinks, so a source that is already squeezed dry keeps its picture (`autoCQPlateauMinSavePercent`).
- **A step has to pay for itself.** Below the low anchor the interpolation is extrapolating, and there the anchor slope is provably too optimistic (measured on a 50 fps source: it promised VMAF 98.0 where the encode delivered 97.5). A **thrift brake** therefore stops the search as soon as one more CQ step buys less than `minGainPerStep` VMAF — 0.30 on the H.265 scale, half that on the finer AV1 one. The target may well be reachable one step further down; it is simply not worth ~7 % file size per step for a difference no eye can find. On the source that prompted it the pick moved from CQ 24 back to CQ 26: 0.35 VMAF given up, 15 % of the file saved.
- **An optional spending limit.** The brakes above all handle the case where more bitrate buys *nothing*. Grainy or very busy material is the opposite problem: the target is reachable, but only at a price — measured on a 1080p50 source, hitting VMAF 97.5 cost 55 % of the original bitrate, while a comparable 30 fps film needed 23 %. `autoCQMaxSourcePercent` (`45` by default, `0` switches it off) puts a ceiling on that spend as a share of the source rate, and the search then takes the cheapest CQ that still fits. Three details make it trustworthy: the source rate is read **at the sampled seconds** rather than as a whole-file average (guided placement sits on the heaviest scenes on purpose); the cost of a CQ step is taken from the **sample encode that already exists** whenever the quality search touched that step, so it is measured rather than modelled; and the search then walks up one step at a time and keeps the first that fits, so the limit takes only what it needs. Untouched steps fall back to an exponential model fitted to the two anchors — good to 1.3 % inside the anchor span, but it drifts once extrapolated past it, which is why a measured sample always wins. The pick is then confirmed by a real encode. It is deliberately **not** the `-maxrate` ceiling: an encoder cap makes several CQ steps measure the same size and the same score, which is exactly the false plateau the saturation brake must never see.
- **The cap keeps out where it cannot help.** Before it changes anything, it checks whether the ceiling is reachable at all — that is, whether even the thriftiest CQ on the scale would come in under it. If not, it leaves the pick alone. This is not a corner case: measured across a real library, sources that were *already* compressed hard need a **higher** share of their own bitrate, not a lower one, because there is nothing left to squeeze out. One 60 fps source at 2.8 Mbit/s stayed at 53 % of itself from CQ 26 all the way to CQ 30, while a 12 Mbit/s source fell below 20 % over the same span. Without this check, a cap tuned for the fat sources would clamp the thin ones to the worst CQ the scale allows **and still miss the cap** — worse picture, no saving. When that happens the log says so, so an unreachable cap can never be mistaken for a file that was simply cheap enough.
...
- **Smart subtitle handling.** Text subtitles are converted to SRT (and cleaned in `-davinci`); bitmap subtitles (PGS, VobSub) that *can't* become text are copied through untouched. Attachments like embedded fonts and cover art ride along on their own.
- **Automatic SRT cleaning.** In `-davinci` mode every extracted SRT has its HTML/ASS styling tags, invisible Unicode junk (soft hyphens, zero-width characters, byte-order marks) and advertising lines stripped — the ad-phrase list is yours to edit in `SRTCleaner_config.txt`, and the file is rewritten atomically. `-split` and `-join` deliberately leave subtitles alone; that is the point of the 1:1 route.
