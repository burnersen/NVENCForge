...
- **An optional spending limit.** The brakes above all handle the case where more bitrate buys *nothing*. Grainy or very busy material is the opposite problem: the target is reachable, but only at a price — measured on a 1080p50 source, hitting VMAF 97.5 cost 55 % of the original bitrate, while a comparable 30 fps film needed 23 %. `autoCQMaxSourcePercent` (`45` by default, `0` switches it off) puts a ceiling on that spend as a share of the source rate, and the search then takes the cheapest CQ that still fits. Two details make it trustworthy: the source rate is read **at the sampled seconds** rather than as a whole-file average (guided placement sits on the heaviest scenes on purpose), and the bitrate-over-CQ curve comes from the two anchor samples that already exist — an exponential model, verified at 1.3 % error against an independent measurement. The pick is then confirmed by a real encode. It is deliberately **not** the `-maxrate` ceiling: an encoder cap makes several CQ steps measure the same size and the same score, which is exactly the false plateau the saturation brake must never see.
